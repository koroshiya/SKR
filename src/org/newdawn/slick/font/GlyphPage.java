...
public class GlyphPage {
	 ...
	 */
	public int loadGlyphs (List<Object> glyphs, int maxGlyphsToLoad) throws SlickException {
		if (rowHeight != 0 && maxGlyphsToLoad == -1) {
			...
			for (Iterator<Object> iter = getIterator(glyphs); iter.hasNext();) {
	 ...
	 */
	private Iterator<Object> getIterator(List<Object> glyphs) {
		if (orderAscending) return glyphs.iterator();
		final ListIterator<Object> iter = glyphs.listIterator(glyphs.size());
